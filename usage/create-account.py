...
def Get_UserName(Headers,Name,Email):
    try:
        ...
        while True:
            ...
            data = {
                'email': Email,
                'name': Name+str(random.randint(7500,7600)),
            }
...
def Create_Acc(Headers,Email,SignUpCode):
    ...
    try:
        ...
        UserName= firstname+'_'+names.get_first_name()+'_'+names.get_last_name()+'_'+str(random.randint(87400,89999)) #Get_UserName(headers,firstname,Email)
        print('UserName', UserName)
        Password=firstname.strip()+'@'+str(random.randint(8500,9500))
        updict = {"referer": 'https://www.instagram.com/accounts/emailsignup/'}
        Headers = {key: updict.get(key, Headers[key]) for key in Headers}
        data = {
